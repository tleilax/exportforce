 ...
 *         http://code.google.com/p/exportforce-class-php/
 ****************************************************************** ** *
 *
 * Copyright (c) 2005-2010 Jan-Hendrik Willms <tleilax@mindfuck.de>
 * Lizensiert unter der MIT-Lizenz.
 *
 ****************************************************************** ** *
 * Diese Datei sollte unter ExportForce.class.php gespeichert werden.
 ****************************************************************** ** *
 * Beispiel (EF- und Klammuser-Daten müssen angepasst werden!):
 ****************************************************************** ** *
 <?php
  $exportforceid = 12345;
  $exportforcepwd = 'gott';
...
class ExportForce
{
	 ...
	 *  - keine -
	 **********************************************************************/
	function ExportForce($ef_id, $ef_pwd, $ef_kennung, $reporterror=false, $logfunc=null)
	{
		$this->_efid = $ef_id;
		$this->_efpwd = $ef_pwd;
		...
		{
			trigger_error('Die übergebene ExportForce-ID kann nicht gültig sein', E_USER_WARNING);
		}
		if (preg_match('~\D~', $ef_kennung) and $this->reporterror)
		{
			trigger_error('Als Kennung muss eine Zahl angegeben werden', E_USER_WARNING);
		}
	}
	...
	function _efQuery($query)
	{
		...
		if (!$fp)
		{
			...
			{
				trigger_error('ExportForce nicht erreichbar', E_USER_NOTICE);
			}
		...
		$request = "GET /{$this->_apipath}{$query} HTTP/1.0\r\n".
			"Host: {$this->_apiurl}\r\n".
			"User-Agent: tlx_ef2class_".VERSION."\r\n".
